...
class AdminReportController extends Controller
{
    ...
    public function getReportProductQty(ReportDateRequest $request)
    {
        ...
        $DateTimeAfterFormat = Carbon::parse($this->DateTimeAfter)->format('Y-m-d H:h:s');;
        // DB::enableQueryLog();
       DB::table("done_orders")
->join("order_details", function($join){
	$join->on("done_orders.order_id", "=", "order_details.order_id");
})
->select("order_details.product_id", "sum (order_details.product_qty) as sl")
->whereBetween("order_details.created_up", "between", $this->DateTimeBefore)
->where($this->DateTimeAfter , .product_id)
