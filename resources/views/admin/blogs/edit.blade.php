...
@section('content')
    ...
    <div class="container-fluid py-4">
        ...
        <div class="card">
            <form class="p-4" action="{{ route('blogs.update',['id'=>$blog->id]) }}" method="post" enctype="multipart/form-data">
                @csrf
                <div class="form-group">
                    <label for="">Tiêu đề bài viết:</label>
                    <input type="text" name="name"
                        class="form-control border border-dark p-2 @error('name') is-invalid @enderror" id=""
                        placeholder="Nhập tên sản phẩm" value="{{ $blog->name ?? old('name') }}">
                    @error('name')
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="slogan"
                        class="form-control border border-dark p-2 @error('slogan') is-invalid @enderror" id=""
                        placeholder="Nhập tên Mô tả" value="{{ $blog->slogan ?? old('slogan') }}">
                    @error('slogan')
                        <div class="alert alert-primary text-white">{{ $message }}</div>
                    @enderror
                </div>
