...
class AdminReportController extends Controller
{
    ...
    public function getReportProductQty(ReportDateRequest $request)
    {
        ...
        $DateTimeAfterFormat = Carbon::parse($this->DateTimeAfter)->format('Y-m-d H:h:s');;
        // DB::enableQueryLog();
        $products =  OrderDetail::whereBetween('updated_at', [$this->DateTimeBefore, $this->DateTimeAfter])
            ->select('product_id', DB::raw('SUM(product_qty) as qty'))
            ->groupBy('product_id')
            ->orderBy('qty','desc')
            ->get();
        // dd(DB::getQueryLog());

        $product_name = OrderDetail::where('product_id', $products[0]->product_id );
        dd($product_name);
