...
class AdminReportController extends Controller
{
    ...
    public function getReportProductQty(ReportDateRequest $request)
    {
        ...
        $productsQty =  OrderDetail::whereBetween('updated_at', [$this->DateTimeBefore, $this->DateTimeAfter])
            ->select('product_id', 'product_name', 'product_discount', 'updated_at', OrderDetail::raw('SUM(product_qty) as qty'))
            ->groupBy('product_id', 'product_name', 'product_discount', 'updated_at')
            ->orderBy('qty','desc')
            ->get();
