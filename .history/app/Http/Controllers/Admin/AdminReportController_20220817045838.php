...
class AdminReportController extends Controller
{
    ...
    public function getReportProductQty(ReportDateRequest $request)
    {
        ...
        $DateTimeBeforeFormat = Carbon::parse($this->DateTimeBefore)->format('Y-m-d H:h:s');;
        $DateTimeAfterFormat = Carbon::parse($this->DateTimeAfter)->format('Y-m-d H:h:s');;
        dd($DateTimeBeforeForDateTimeBeforeFormatmat);
        DB::enableQueryLog();
        $products =  DB::table("done_orders")
            ->join("order_details", function ($join) {
                $join->on("done_orders.order_id", "=", "order_details.order_id");
            })
            ->select('order_details.product_id')->sum('order_details.product_qty')
            ->whereBetween('created_up', [$DateTimeBeforeFormat, $DateTimeAfterFormat])
            ->groupBy("order_details")
            ->get();
        dd(DB::getQueryLog());
        // dd($products);
