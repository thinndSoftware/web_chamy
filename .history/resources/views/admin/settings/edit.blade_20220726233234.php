...
@section('content')
    ...
    <div class="container-fluid py-4">
        ...
        <div class="card">
            <form class="p-4" action="{{ route('settings.update').'?type='.$settings->type }}" method="post">
                @csrf
                <div class="form-group">
                    <label for="">Config key:</label>
                    <input type="text" name="config_key" class="form-control border border-dark p-2" id=""
                        placeholder="Nhập Config key " value="{{ $settings->config_key?:old('config_key') }}">
                    @error('config_key')
